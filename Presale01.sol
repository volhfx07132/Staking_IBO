...
contract Presale01 is ReentrancyGuard {
    ...
    // VestingPeriod private VESTING_PERIOD;
    mapping(address => bool) public admins;

    uint256 public TOTAL_FEE;
    uint16 public PERCENT_FEE;
    ...
    VestingInfo[] public VESTING_INFO;
    ...
    function init3(
        ...
        uint256 _refundFee,
        uint256 _refundTime,
        uint256[] memory _refundTimes,
        uint32[] memory _unlockRates
    ) external {
        require(msg.sender == PRESALE_GENERATOR, "FORBIDDEN");
        require(_refundTimes.length == _unlockRates.length, "INVALID REFUND");
        ...
        // vesting info
        for (uint256 i = 0; i < _refundTimes.length; i++) {
            VESTING_INFO.push(VestingInfo(_refundTimes[i], _unlockRates[i]));
        }
    }
    ...
    modifier onlyPresaleOwnerOrAdmin() {
        require(
            PRESALE_INFO.PRESALE_OWNER == msg.sender || admins[msg.sender],
            "NOT PRESALE OWNER OR ADMIN"
        );
        _;
    }
    ...
    function userDeposit(
        ...
        if (real_amount_in > allowance) {
            ...
            if (!STATUS.WHITELIST_ONLY) {
                fee = (real_amount_in * PERCENT_FEE) / (1000 - PERCENT_FEE);
            }
        }
    ...
    function userWithdrawTokens() external nonReentrant {
        ...
        uint256 length = VESTING_INFO.length;
        uint256 tokensOwed = buyer.tokensOwed;
        uint256 totalUnlockRate = 0;
        uint256 lastWithdraw = 0;

        for (uint256 i = 0; i < length; i++) {
            if (
                block.timestamp > VESTING_INFO[i].vestingTime &&
                buyer.lastWithdraw < VESTING_INFO[i].vestingTime
            ) {
                totalUnlockRate += VESTING_INFO[i].unlockRate;
                lastWithdraw = block.timestamp;
            }
        }

        if (totalUnlockRate > 0) {
            buyer.lastWithdraw = lastWithdraw;
            uint256 amountWithdraw = (tokensOwed * totalUnlockRate) / (1000000); // 4 decimals
    ...
    function listOnUniswap() external onlyCaller {
        ...
        require(
            block.timestamp >
                PRESALE_INFO.UNISWAP_LISTING_TIME +
                    REFUND_INFO.refundTime *
                    1 hours,
            "EARLY TO CALL"
        );
        // require(
        ...
        if (PRESALE_INFO.ADD_LP == 2) {
            ...
            TransferHelper.safeTransferBaseToken(
                ...
                STATUS.TOTAL_BASE_COLLECTED + TOTAL_FEE - TOTAL_TOKENS_REFUNDED,
                !PRESALE_INFO.PRESALE_IN_ETH
            );

            // send all token
            uint256 tokenBalance = PRESALE_INFO.S_TOKEN.balanceOf(
                address(this)
            );
            TransferHelper.safeTransfer(
                address(PRESALE_INFO.S_TOKEN),
        ...
        uint256 DAOLaunchBaseFee = ((STATUS.TOTAL_BASE_COLLECTED -
            TOTAL_TOKENS_REFUNDED) * PRESALE_FEE_INFO.DAOLAUNCH_BASE_FEE) /
            1000;
        // base token liquidity
        uint256 baseLiquidity = (((STATUS.TOTAL_BASE_COLLECTED -
            TOTAL_TOKENS_REFUNDED) - DAOLaunchBaseFee) *
            PRESALE_INFO.LIQUIDITY_PERCENT) / 1000;
        if (
    ...
    function ownerWithdrawTokens() external nonReentrant onlyPresaleOwner {
        ...
        uint256 tokenRefunded = (baseRefund *
            PRESALE_INFO.TOKEN_PRICE) /
            (10**uint256(PRESALE_INFO.B_TOKEN.decimals()));
    ...
    function userRefundTokens() external nonReentrant {
        ...
        uint256 whitelistDeposited = buyer.baseDeposited -
            (USER_FEES[msg.sender] * 1000) /
            PERCENT_FEE;
        uint256 refundAmount = (whitelistDeposited *
            (1000 - REFUND_INFO.refundFee)) / 1000;
        require(refundAmount > 0, "NOTHING TO REFUND");

        TOTAL_TOKENS_REFUNDED += refundAmount;
        uint256 tokensRefunded = (whitelistDeposited *
            PRESALE_INFO.TOKEN_PRICE) /
            (10**uint256(PRESALE_INFO.B_TOKEN.decimals()));
    ...
    function finalize() external {
        ...
        if (!PRESALE_INFO.PRESALE_IN_ETH) {
            remainingBBalance = PRESALE_INFO.B_TOKEN.balanceOf(address(this));
        } else {
    ...
    // editable at any stage of the presale
    function changePresaleType(bool _flag, uint256 _maxSpend)
        public
        onlyAdmin
    {
        STATUS.WHITELIST_ONLY = _flag;
    ...
    function updateListingTime(uint256 _listOnUniswap) public onlyAdmin {
        require(_listOnUniswap > PRESALE_INFO.END_TIME, "INVALID TIME");
        PRESALE_INFO.UNISWAP_LISTING_TIME = _listOnUniswap;
    }

    function updatePresale(
        bool _isUpdateListingTime,
        uint256 _listOnUniswap,
        bool _isUpdateWhitelist,
        bool _flag,
        uint256 _maxSpend,
        bool _isUpdateVesting,
        uint256[] memory _vestingTimes,
        uint32[] memory _unlockRates,
        bool _isUpdateRefund,
        bool _isRefund,
        uint256 _fee,
        uint256 _time
    ) external onlyAdmin {
        if (_isUpdateListingTime) {
            updateListingTime(_listOnUniswap);
        }

        if (_isUpdateWhitelist) {
            changePresaleType(_flag, _maxSpend);
        }

        if (_isUpdateVesting) {
            require(
            _vestingTimes.length == _unlockRates.length &&
                _vestingTimes.length > 0,
                "LENGTH ERROR"
            );
            delete VESTING_INFO;
            for (uint256 i = 0; i < _vestingTimes.length; i++) {
                VestingInfo memory newVestingPeriod;
                newVestingPeriod.vestingTime = _vestingTimes[i];
                newVestingPeriod.unlockRate = _unlockRates[i];

                VESTING_INFO.push(newVestingPeriod);
            }
        }

        if (_isUpdateRefund) {
            require(_fee <= 200, "MAX REFUN FEE");
            require(_time >= 1 && _time <= 24, "INVALID TIME");

            REFUND_INFO.isRefund = _isRefund;
            REFUND_INFO.refundFee = _fee;
            REFUND_INFO.refundTime = _time;
        }
    }
}
